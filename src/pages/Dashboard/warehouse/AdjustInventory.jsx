...
const AdjustInventory = () => {
  ...
  const displayColumns = [
    ...
    { key: "createdAt", label: "Ngày điều chỉnh" },
    { key: "statusName", label: "Lý do điều chỉnh" },
          ...
          try {
            const [sourceWarehouse, destinationWarehouse, product, status] =
              await Promise.all([
                ...
                getInventoryAdjustmentTypeById(transfer.statusId),
              ]);
            ...
            return {
              ...
              statusName: status?.name || "Lý do không rõ",
            };
          } catch (innerErr) {
            console.error(
              `Lỗi khi lấy thông tin chi tiết cho điều chỉnh ${transfer.id}:`,
              innerErr
            );
            return {
              ...transfer,
  ...
  const handleViewDetail = (id) => {
    navigate(`/dashboard/warehouse/adjust-inventory-detail/${id}`);
  };
  ...
  const handleDelete = async (item) => {
    if (window.confirm("Bạn có chắc muốn xóa điều chỉnh tồn kho này?")) {
      try {
        await deleteWarehouseTransfer(item.id);
        await fetchData();
        toast.success("Xóa điều chỉnh tồn kho thành công");
      } catch (error) {
        console.error("Lỗi khi xóa điều chỉnh tồn kho:", error);
        toast.error("Lỗi khi xóa điều chỉnh tồn kho");
      }
    }
  };
